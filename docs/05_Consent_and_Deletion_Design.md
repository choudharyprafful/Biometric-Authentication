...
| `trainingConsentGiven` / `trainingConsentAt` | `POST /auth/register` with `trainingConsent: true` (optional, defaults to false), or `POST /users/me/training-consent` with `consent: true` afterward | `POST /users/me/training-consent` with `consent: false` — freely toggleable either direction, any time, regardless of what was chosen at registration. **Known defect, found 2026-09-25 (docs/04 R-CONSENT-2):** for an account that has not enrolled MFA yet, or is awaiting parental consent, this endpoint (and `POST /users/me/content-personalization-consent`) currently returns `403` because an MFA gate from another router leaks onto it — so "any time" does not yet hold for those accounts | Nothing — gates whether activity contributes to the behavior model (§5b), not any core feature |
