...
const TYPE_SCALE = [
  { label: 'Display', className: 'text-4xl font-bold font-mono tracking-widest uppercase' },
  { label: 'Heading', className: 'text-2xl font-semibold font-mono tracking-wide uppercase' },
  { label: 'Body', className: 'text-base font-sans' },
  { label: 'Label', className: 'text-sm font-medium font-mono tracking-wider uppercase' },
  { label: 'Caption', className: 'text-sm text-muted-foreground font-sans' },
] as const;
  ...
  return (
    <div className="space-y-8 rounded-xl border bg-card p-6 text-card-foreground">
      <section className="grid gap-6 sm:grid-cols-2">
        <div>
          <h2 className="text-xs font-medium uppercase tracking-widest text-muted-foreground font-mono">
            JetBrains Mono — technical labels
          </h2>
          <p className="mt-4 text-3xl font-bold font-mono tracking-widest uppercase">
            SECUREAI
          </p>
          <p className="mt-2 text-sm text-muted-foreground">
            All headings, category tags, badge text, log entries, numeric readouts, and UI labels.
        ...
        <div>
          ...
          <p className="mt-4 text-3xl font-bold font-sans">
            Identity Verification
          </p>
          <p className="mt-2 text-sm text-muted-foreground">
            Body text, descriptions, help copy, and any prose that needs comfortable reading weight.
          </p>
        </div>
      </section>
      ...
      <section className="space-y-4 border-t pt-6">
        ...
        {TYPE_SCALE.map((entry) => (
          <div key={entry.label} className="grid gap-2 sm:grid-cols-[96px_1fr]">
            <span className="pt-1 text-xs font-medium uppercase tracking-widest text-muted-foreground font-mono">
              {entry.label}
            </span>
            <p className={entry.className}>
              {entry.label === 'Body' || entry.label === 'Caption'
                ? 'Biometric authentication protects operator access.'
                : 'IDENTITY VERIFIED'}
            </p>
          </div>
        ))}
