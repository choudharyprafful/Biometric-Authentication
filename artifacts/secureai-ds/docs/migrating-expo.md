# Migrating Expo UI to SecureAI Design System

Read `artifacts/secureai-ds/docs/AGENTS.md` and
`artifacts/secureai-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/secureai-ds/lib/native-theme` and
  `@workspace/secureai-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/secureai-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/secureai-ds` and retains only
