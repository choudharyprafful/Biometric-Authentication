# Consuming SecureAI Design System in web apps

Read `artifacts/secureai-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/secureai-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/secureai-ds/styles.css";
```
...
`node_modules/@workspace/secureai-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/secureai-ds/components/ui/button";
import { cn } from "@workspace/secureai-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/secureai-ds/hooks/use-toast";
```
...
`@workspace/secureai-ds/components/ui/button`. Run the app's typecheck
